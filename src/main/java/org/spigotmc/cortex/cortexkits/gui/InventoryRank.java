...
public class InventoryRank extends Pagination {

    public InventoryRank(GuiLibrary guiLibrary) {
        super(guiLibrary);
    }

    @Override
    public String getMenuName() {
        return new ColoredString(DataManager.getPrefix() + " &2&oRank kits", ColoredString.ColorType.MC).toString();
    }
        ...
        if (kits != null && !kits.isEmpty()) {
            for (int i = 0; i < getMaxItemsPerPage(); i++) {
                ...
                if (kits.get(index) != null) {
                    if (KitAPI.getCategory(kits.get(index)) != null && KitAPI.getCategory(kits.get(index)).equals("Rank")) {
